...
import { CREATE_USER, GET_USER_BY_EMAIL } from '../../../lib/graphql/operations';
import { hashPassword, generateSessionToken } from '../../../server/utils/auth';
import { 
	RegisterRequest, 
	AuthResponse,
	EmailSchema,
...
export default defineEventHandler(async (event) => {
  ...
  const { data: existingUser, error: checkError } = await executeQuery<{ users: any[] }>(
    GET_USER_BY_EMAIL,
  ...
  // Create user
  const { data: result, error: mutationError } = await executeMutation<{ insert_users_one: any }>(
    CREATE_USER,
    {
      email: email.toLowerCase(),
