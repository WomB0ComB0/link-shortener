...
export class UrlValidationResult extends S.Class<UrlValidationResult>("UrlValidationResult")({
	isValid: S.Boolean,
	url: S.String,
	protocol: S.String,
	domain: S.String,
 ...
 */
function parseDomain(hostname: string): { domain: string; tld: string | null; subdomain: string | null } {
...
export async function validateUrl(url: string): Promise<UrlValidationResult> {
	...
	if (!["http", "https"].includes(protocol)) {
		errors.push(`Invalid protocol: ${protocol}. Only HTTP and HTTPS are allowed`);
	}
	...
	if (!hostname) {
		...
		if (
			...
			errors.push("URLs pointing to localhost or private networks are not allowed");
		}
	...
	const foundSuspiciousKeywords = SUSPICIOUS_KEYWORDS.filter(keyword => 
		urlLower.includes(keyword) && !domain?.toLowerCase().includes(keyword)
	);
	if (foundSuspiciousKeywords.length > 0) {
		warnings.push(`URL contains suspicious keywords: ${foundSuspiciousKeywords.join(", ")}`);
	}
	...
	if (/[^\x00-\x7F]/.test(hostname)) {
		warnings.push("Domain contains non-ASCII characters (possible homograph attack)");
	}
